  ...
  return (
    <>
      <Row className={`${width > 768 ? "mt-24" : "mt-20"}`}>
        <Col
          xs={24}
          className={`text-center ${width > 768 ? "space-y-10" : "space-y-8"}`}
        >
          ...
          >
            <span className="text-stroke-primary">{"<"} </span> FullStack Developer{" "}
            <span className="text-stroke-primary"> {"/>"}</span>
          </h2>
          <Button type="primary">
            <a
              href="https://drive.google.com/file/d/11PIKfoAk_-Dqq25N2FlSqr4uYqjc-XUs/view?usp=drivesdk"
              target="_blank"
            >
              View Resume
            </a>
      ...
      <div
        className={`flex justify-center ${width > 768 ? "-mt-14" : "-mt-16"}`}
      >
        <motion.div
          animate={{ y: [0, -20, 0] }}
          transition={{
        ...
        >
          <img src={img} alt="home-image" width={width > 768 ? "450" : "300"} />
        </motion.div>
      </div>
    </>
  );
